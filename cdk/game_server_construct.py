...
class GameServerConstruct(Construct):
    cluster: ecs.Cluster
    valheim_task: ecs.FargateTaskDefinition

    def __init__(self, scope: Construct, construct_id: str, vpc, file_system, **kwargs) -> None:
        super().__init__(scope, construct_id)

        self.cluster = ecs.Cluster(self, "GameServerCluster", vpc=vpc, cluster_name="GameServerCluster")
        ...
        self.valheim_task = self.get_valheim_task(server_volume, mount_point)

        service_security_group = ec2.SecurityGroup(self, "GameServerSecurityGroup", vpc=vpc,
                                                   security_group_name="GameServerSecurityGroup")

        service = ecs.FargateService(self,
                                     "GameServerFargateService",
                                     cluster=self.cluster,
                                     task_definition=self.valheim_task,
                                     desired_count=0,
